...
## 14.07.2026 — Build UI and Aspect-Ratio Fixes

Building the game surfaced two display problems that never showed in the editor at the default Game-view size. The first was the UI: the in-game HUD was scattered and the "FLOOR 1" transition text was tiny. The cause was two canvases — the in-game HUD and the transition screen — left on the Canvas Scaler's "Constant Pixel Size" mode with an 800x600 reference, while every other canvas used "Scale With Screen Size" at 1920x1080. Constant Pixel Size lays the UI out in raw pixels, so it looked right in a small editor window but broke at the build's real resolution. Setting both canvases to Scale With Screen Size at 1920x1080 (match 0.5), matching the rest, fixed it.

The second problem was the game view itself not fitting once the window could be an arbitrary shape. The camera uses the `CameraAspect` script, which letterboxes the view to a fixed 16:9 by shrinking the camera's viewport rect — the right approach for rooms designed at a fixed aspect. The bug was that it only computed the rect once, in Awake, and never again. So the moment the window changed shape (the editor's Free Aspect, or a resizable build window), the viewport went stale and the game no longer fit. The fix was to pull the calculation into its own method and call it from Update whenever Screen.width or Screen.height changes, so the letterbox always tracks the current window. Now the game renders as a correctly-fitted, centred 16:9 area with black bars on the leftover space at any window shape, while the screen-space UI continues to fill the whole window on top.

---

## 15.07.2026 — Player Could Act During the Return-to-Menu Fade

Found a small but visible bug: pausing mid-run and hitting "Return to Main Menu" left the character able to rotate and shoot during the fade-to-black. The cause was a timing gap. `ReturnToMainMenu` clears `IsPaused` right away, but the actual teardown that destroys the player only runs in the transition's on-black callback, half a second later. In between, the player still existed and `PlayerMovement.Update` gated aiming and attacking purely on `IsPaused` — which was now false — so the player could act during the fade.

The fix tightened that gate. Facing and attacking are now allowed only when the player is both unpaused and has `canMove` set, and `ReturnToMainMenu` sets `canMove` to false the instant it's called. `canMove` is the flag already used to lock the player during transitions, so this also closed the same hole during floor transitions, where the player could previously fire while the screen was black. Movement was never affected because it already respected `canMove` in FixedUpdate; this just brings the aim/attack path in line with it.
